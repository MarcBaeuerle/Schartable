...
export default function Nav({ test } :any) {
    ...
    const logHandler = ():void => {
        test ? logout() : window.location.href = LOGIN_URI;
        return;
    }
    ...
    return (
        <>
            <nav className={`${openMenu ? ' opacity-100 ' : ' opacity-0 pointer-events-none '} duration-500 z-10 ease-in-out flex flex-col md:hidden text-blue-950 font-rale font-bold text-xl absolute w-full h-fit bg-red-100 p-7 shadow-xl text-center`}>
                {home}
                {privacy}
                {log}
            </nav>
            <nav className="flex justify-between p-7 font-rale font-semibold text-center items-center text-blue-950 text-xl">
                ...
                <div className="hidden md:flex gap-3 h-fit items-center">
                    {home}
